...
with Diagram(
    ...
    # Entry
    user >> Edge(label="HTTPS") >> cloudflare >> nlb >> istio_ing
    istio_ing >> fe_pod >> Edge(label="HTTP") >> mw_pod
    ...
    for w in [fe_pod, mw_pod, be_pod, kserve_pod]:
        ...
        w >> Edge(label="traces", **subtle) >> jaeger
    ...
    istio_ing >> Edge(label="mesh viz", **subtle) >> kiali
